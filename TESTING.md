# Testing RustedWax v0.5.0

`dist/rustedwax-0.5.0.apk` — detection, key handling, automatic scrobbling
...
```bash
~/Library/Android/sdk/platform-tools/adb install -r dist/rustedwax-0.5.0.apk
```
...
84 tests: 17 crypto/serialization vectors generated by dhive (the library
Keychain signs with), 16 title-parsing, 21 kind-classification (including the
eleven misclassified videos from the 2026-07-24 field sample as pinned
regressions), 10 threshold/double-listen, 8 notification-hint binding, 6
watch-page extraction, 6 MusicBrainz matching.
...
- **`url` needs the address-bar watcher.** Without it (or when the id can't be
  latched in time), scrobbles carry `platform` but no `url` — and no category
  or MusicBrainz check, so `kind` falls back to the offline heuristics.
...
- **`Title | Channel`-shaped titles** can split as artist|track (a kitten video
  broadcast with the fields swapped). Harmless while `kind: video`; open item.
...
- Stale/cross-tab site hints (per-session binding + taint, v0.4.0); the
  finalize-time address-bar race (id latching, v0.4.2); trailers and film
  clips scrobbled as `song` (evidence-layered classifier, v0.4.1); looping
  shorts double-scrobbling (song-only double-listen, v0.5.0).
