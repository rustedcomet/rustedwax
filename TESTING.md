# Testing RustedWax v0.6.1

`dist/rustedwax-0.6.1.apk` — detection, key handling, automatic scrobbling
...
```bash
~/Library/Android/sdk/platform-tools/adb install -r dist/rustedwax-0.6.1.apk
```
...
| Tap **Broadcast this scrobble**, then let the track finish | **One** scrobble. The manual send claims the ledger, so the automatic finalize is blocked — Log says "already scrobbled" |
...
119 tests: 17 crypto/serialization vectors generated by dhive (the library
...
address-bar corroboration, 10 search-based and 8 playlist-based id resolution,
7 dedup-key, 6
watch-page extraction, 6 MusicBrainz
matching.

## Known gaps in v0.6.1
...
- **`url` is now best-effort in two stages.** The address bar is exact when it
  reports, but it goes silent for long stretches — a playlist advancing behind a
  hidden toolbar fires no accessibility event at all. Since v0.6.0 the app falls
  back to searching YouTube for the session's title + channel; that match
  requires title, channel *and* duration to agree, so an unverifiable one still
  leaves `url` unset rather than guessing. A missing `url` logs its own reason
  (`broadcasting WITHOUT url …`, `no confident match …`). Both stages need
